...
class StudentUSocket(StudentUSocketBase):
  ...
  def tx(self, p, retxed=False):
    ...
    if p.tcp.payload and not retxed:
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    while not self.rx_queue.empty():
      packet = self.rx_queue.peek()[1]
      if packet.tcp.seq |GT| self.rcv.nxt:
        self.set_pending_ack()
        break
      else:
        self.rx_queue.pop()
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
      ...
      left_widow_space = snd.wnd |MINUS| in_flight
      if left_widow_space <= 0:
        break

      len_payload = min(left_widow_space, self.mss, remaining)

      p = self.new_packet()
      p.tcp.payload = self.tx_data[:len_payload]
      self.tx(p)
      self.tx_data = self.tx_data[len_payload:]

      num_pkts += 1
      bytes_sent = bytes_sent |PLUS| len_payload
      remaining = remaining |MINUS| len_payload
